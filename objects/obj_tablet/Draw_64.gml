...
if (tabs[current_tab] == "Conceitual") {
	...
	items_visiveis_na_lista = max(1, floor(content_h / line_h)); // << MODIFICADO (Remove 'var')

	// >> NOVO: O loop agora começa do item 'conceptual_scroll_top' e desenha apenas os itens visíveis
	var end_index = min(array_length(items), conceptual_scroll_top + items_visiveis_na_lista); // << MODIFICADO
