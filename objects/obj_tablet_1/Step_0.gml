...
// coletar objetivos da sala
var day_items = tablet_get_objectives_for_room();

// assume que todos prontos até prova em contrário
var all_done = true;

// percorre, mas podemos sair cedo ao achar um não-completo
for (var j = 0; j < array_length(day_items); ++j) {
    var it = day_items[j];
    if (!tablet_is_objective_complete(it.flag)) {
        all_done = false;
        break; // já sabemos que não está tudo completo => sai do loop
    }
}

// só após o loop decidimos mostrar/resetar
if (array_length(day_items) > 0) {
    if (all_done) {
        // só dispara o popup + som na transição (quando ainda não mostramos)
        if (!objectives_popup_shown) {
            show_popup("Nível concluído!", 3);
            audio_play_sound(snd_puzzle_solved, 0, false, 1, 0, 1);
			objectives_popup_shown = true;
        }
    } else {
        // se algum objetivo voltou a ficar incompleto, permite mostrar futuramente
